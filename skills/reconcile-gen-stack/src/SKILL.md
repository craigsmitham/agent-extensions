...
description: Assesses or reconciles an established OKF v0.2 Gen Stack corpus against the current profile, including cross-cutting governance, Intent, Architecture, subject-colocated Requirements, and ADRs. Use for review, maintenance, repair, refresh, cleanup, pruning, authorized migration, broken navigation, staleness, duplication, contradiction, or nonconformance. Not for initial adoption, inventing a new concept, choosing architecture, changing implementation, or treating evaluation and operational artifacts as corpus concepts.
---
...
3. **Trace the corpus.** Follow root navigation through the five cross-cutting
   kernel concepts, Intent, Architecture, Requirements, and ADRs. Inspect only
...
7. **Assess corpus boundaries.** Corpus `implementation/`, `evaluations/`,
   `feedback/`, `signals/`, and `observations/` collections are
   nonconforming. Preserve their useful artifacts at repository-native or
   runtime authorities and repair links; do not delete evidence merely because
   it is outside the corpus.
...
10. **Verify.** Re-run reachability, canonical-path, colocation, authority, and
    absence-of-prohibited-collection checks. Run the established OKF check and

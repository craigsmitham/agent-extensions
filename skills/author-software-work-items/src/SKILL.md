...
6. **Analyze Requirement, architecture, and evidence impact.** Apply the Gen
   ...
   non-satisfaction, candidate new obligation, proposed change or retirement,
   implementation-only, evidence or interpretation gap, or unresolved. Name
   affected architecture responsibility, boundary, decision, or response only
   from supplied authority. Name evaluations that may need creation,
   correction, rerun, or reinterpretation. Identify the authority needed to
   accept or resolve a change. This analysis records relationships; it MUST
   NOT create, accept, revise, or retire a Requirement or design architecture.
   A test or evaluation may intentionally repeat a Requirement predicate while
   remaining a distinct witness; do not remove that useful redundancy or treat
   the evaluation as normative authority.
7. **Derive the brief last.** Write a discriminating title and a one- or
...
8. **Apply only the authorized external change.** Before filing or updating,
   ...
   process in scope.
9. **Read back external writes.** After a create or update, retrieve the
...
10. **Verify and hand off.** Compare the final draft, or the persisted readback
   ...
   the Requirement-impact classification and unresolved authority,
   architecture, or evidence consequences, and name any material the host
   could not represent.
...
explicitly unavailable, supplied context has not been lost, Requirement impact
is explicit or honestly unresolved, and no unaccepted product, Requirement,
design, delivery, or priority decision has been smuggled into it.

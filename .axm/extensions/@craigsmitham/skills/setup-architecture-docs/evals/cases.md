...
local structure, explicit federation of multiple system corpora, read-only
adoption assessment, ambiguous system boundaries, resistance to empty
taxonomies and invented architecture, optional OKF adoption, outcome-based
verification, and the boundary between setup and maintenance. They also cover
...
local choices, preserves existing material, makes consequential ambiguity a
real decision gate, and defers substantive authoring or established-corpus
maintenance to their owning workflows. A read-only pass additionally requires
observable no-change evidence within the harness boundary; missing evidence is
unknown, not success.

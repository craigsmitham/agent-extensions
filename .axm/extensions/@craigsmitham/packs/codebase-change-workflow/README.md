...
through accepted design and planning, then verify the completed change. The pack
preserves evidence provenance and checks relevant code, configuration,
dependency, deployment, and runtime drift across six distinct stages:
...
6. Verify the completed implementation against its accepted contract and snapshot.
...
choices, functional scenarios, technical contracts, delivery steps, or
completion evidence need to be made explicit. Implementation itself occurs
outside the pack between planning and verification. The pack does not modify
code, estimate delivery, or create vendor-specific work items.
...
coding agent or translation into a work management system. After implementation,
verify the accepted obligations against the actual change and objective evidence.

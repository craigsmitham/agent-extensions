...
For a practical authoring process, see [Expressing invariants](expressing-invariants.md).
...
- encapsulated domain operations;
- static dependency or policy checks;
- assertions and runtime validation;
- property-based and contract tests;
- model checking or proof; and
- monitoring that detects violations after they occur.

Mechanisms differ in strength. A database constraint can reject a violating
write within its supported scope; a sampled test can reveal counterexamples but
does not prove a universal claim; monitoring detects rather than prevents; a
proof establishes only the modeled property under its assumptions.

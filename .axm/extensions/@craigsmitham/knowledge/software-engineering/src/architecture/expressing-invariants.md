...
Avoid an unqualified “always.” If temporary inconsistency is permitted inside
an atomic operation, say so by naming the stable boundary. If a relationship is
restored only after asynchronous reconciliation, document a local invariant
and a convergence requirement instead of claiming a global invariant.
...
Do not expose proof-only auxiliary invariants as domain requirements unless
they are themselves meaningful obligations. A verifier may need a stronger
inductive invariant than the stakeholder-facing property.
...
Record what the mechanism actually guarantees. A test is evidence over tested
cases, monitoring detects rather than prevents, and formal results are bounded
by the model and assumptions.

Let prose own the invariant's meaning, boundary, rationale, and source. Link to
the executable mechanism that owns detailed enforcement logic rather than
maintaining two independently editable formulas.

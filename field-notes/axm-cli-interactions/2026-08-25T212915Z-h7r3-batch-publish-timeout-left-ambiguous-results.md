...
**Recovery:** Read every affected package from the Registry, selected only `@craigsmitham/skills/reconcile-gen-stack@0.5.0` and `@craigsmitham/knowledge/software-architecture@4.1.0`, then used a fresh preview and `--on-existing verify` so any late completion would become an integrity-checked no-op. Both exact versions subsequently published successfully.

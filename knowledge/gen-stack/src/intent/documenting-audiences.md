...
description: Use when a durable, evidence-supported group affected by or receiving value from an Offering needs explicit identity; create one Audience concept without publishing private people, accounts, or research personas.
...
sources:
  ...
  - resource: ../profile/gen-stack-application-profile.md#audience
    title: Gen Stack application profile — Audience
...
2. Create its canonical file using the `Audience` type and common fields from
   the [application profile](../profile/gen-stack-application-profile.md#audience).
...
6. Link related offerings, needs, jobs, propositions, use cases, features, or
   surfaces in prose, then add the concept to `intent/audiences/index.md`.

...
description: Use when a suspected discrepancy needs an attributable, evidence-bearing record; preserve its source and expectation, add proportional evidence, and maintain classification, resolution, relationships, and verification without inventing diagnosis or priority.
...
The report remains that provenance-bearing record throughout its lifecycle; it
is never retitled as corrective work.
...
preferred fix. Keep the title evidence-facing as investigation progresses: it
may become more precise about the observed discrepancy, but it does not become
the corrected-behavior title of later work. For example, do not rename the
report above to `Preserve zero-value invoice lines when tax detail is
exported`; create and link that separate Bugfix Specification when correction
is authorized.

Do not prefix the title with `Defect report` or put a Bugfix identifier in it
when the surrounding tracker surface already shows the type and relationships.
Add a one- or two-sentence summary stating the discrepancy, its impact, and
...
investigation contains constraints, proposals, architecture or code sketches,
tradeoffs, or open questions, preserve an attributed synopsis or link their
authoritative home. The report may record that a corrective decision or plan
exists, but accepted corrective design and delivery context belong in a
separate Bugfix Specification. See
...
Do not retitle, replace, or move the Defect report's evidence into it.

## 12. Define verification conditions

State the observable evidence needed to classify or close the report. When a
Bugfix Specification exists, it owns the behavioral conditions for verifying
the corrective change; the report links those conditions and later records the
relevant result without becoming the correction specification.
...
### Identified Bug and corrective-work relationships

- Identified Bugs:
- Bugfix Specifications or other corrective work:
- Attributed proposal or design synopsis, when useful:
- Authoritative design or delivery links:
...
- Existing proposal or technical context is attributed and linked without
  placing accepted corrective design or delivery authority in the report.
- No Defect report was retitled, replaced, or treated as the Bugfix
  Specification.

...
## Artifact meaning determines what the title names

Titles do not merely describe a shared topic. They expose the primary meaning
of the artifact on reading surfaces that may show a Defect report beside the
corrective work it informed.
...
The relationship belongs in links or structured fields. A title such as `Fix
defect report #482` spends its distinguishing surface on a tracker identifier
and says nothing about the correction. Likewise, `Defect report:` and
`Bugfix:` prefixes add noise when the surrounding surface already displays the
item type.

...
option before the information and need that give the choice its value.[^beck-yagni-timing]
...
Beck describes it more precisely as a question of timing.[^fowler-yagni][^beck-yagni-timing]
...
- paying for structure before it enables value worsens the timing of the
  investment.[^metz-wrong-abstraction]
...
earlier investigation and proportionate preparation.[^aws-review-process]
...
[Tidy First](../patterns/tidy-first.md) pattern to make only the preparatory
change that enables today's behavior change.
...
- **Simple and incremental design** are practices within which YAGNI is
  exercised and corrected through feedback.[^agile-simple-design]
- **Wrong abstraction** is a common consequence of violating the principle by
  deciding too early which cases belong together.[^metz-wrong-abstraction]
...
[^beck-yagni-timing]: Beck calls YAGNI a meditation on timing and grounds the cost of
    speculative structure in lost optionality and unfavorable economic timing.
[^fowler-yagni]: Fowler traces the phrase to Beck and Hendrickson and relates it to
    XP's Simple Design and incremental-design practice.
[^metz-wrong-abstraction]: Metz shows how premature shared abstractions accumulate parameters and
    conditionals when later cases vary differently than expected.
[^aws-review-process]: AWS uses reversible and irreversible decisions to distinguish the
    amount and timing of analysis a choice warrants.
[^agile-simple-design]: The Agile Alliance describes simple design as continuous design in

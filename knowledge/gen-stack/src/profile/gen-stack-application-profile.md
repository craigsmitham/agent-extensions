...
description: The application profile for a durable human-authored corpus at repository `gen-stack/`, governing cross-cutting system governance, Intent, Architecture, subject-colocated Requirements, and a discoverable System Evaluation Approach.
...
subject-colocated Requirements, and one System Evaluation Approach. The
...
Implementation, concrete Evaluation artifacts, Signals, and Observations are
peer authorities outside this corpus contract. A conforming corpus MUST NOT
require `implementation/`, `feedback/`, `signals/`, or `observations/`
collections or copy concrete Evaluation Definitions, Suites, Executions,
Results, or Reports into the corpus.

A conforming corpus MUST be an OKF v0.2 bundle. Its root `index.md` MUST state
that it adopts and link `gen-stack` version `0.4.0`. This is an open-world
...
Evaluation, Signal, Observation, and work-item authorities remain outside the
corpus unless another authority independently places them there.
...
Evaluation Definitions, Executions, Results, and coverage claims.
...
A conforming corpus MUST contain the five required root concepts and the System
Evaluation Approach, use governed types at canonical paths, keep every
...
`evaluations/system-evaluation-approach.md` with exact type
`System Evaluation Approach`, mapped to
[`system-evaluation-approach`](../glossary.md#term-system-evaluation-approach).
[Evaluation as bounded evidence](../evaluations/evaluation-as-bounded-evidence.md)
provides the primary explanation; [Designing a System Evaluation
Approach](../evaluations/designing-a-system-evaluation-approach.md) is the
authoring guide.
...
Architecture tree. It MUST instead make current evidence navigable through the
canonical subject hierarchies and cross-view relationships. For maintained
...
### Peer-owned relationship encodings

The remaining controlled relationships are semantically defined by the
glossary but have an assertion source outside this corpus contract. This
profile MUST NOT require their peer artifacts to move into the corpus or place
volatile backlinks on governed concepts.
...
| [`evaluation-definition-evaluates-requirement`](../glossary.md#relationship-evaluation-definition-evaluates-requirement) | The repository-native Evaluation Definition |
| [`evaluation-definition-evaluates-architecture-realization`](../glossary.md#relationship-evaluation-definition-evaluates-architecture-realization) | The repository-native Evaluation Definition |
...
To migrate an explicitly identified `gen-stack` `0.2.0` or `0.3.0` corpus or a
supported corpus previously stored elsewhere:
...
4. update the root adoption statement to `0.4.0`;
5. synchronize relationships safely established by canonical paths and the
   existing Requirement fields;
6. manually encode consequential controlled relationships previously carried
   only by prose or ordinary links;
7. synchronize reciprocal projections;
8. update repository-external links, continuous-integration paths, harness
   inputs, and agent instructions that named the old location; and
9. run OKF validation against `gen-stack/`, profile validation against the
...
and range, cardinality, path consistency, reciprocal projection, and cycles
where the glossary prohibits them. Also run the applicable OKF validator
...
Requirement subject selection, completeness, satisfaction, coverage, or
fitness.

...
tags: [codebase-review, software-quality, quality-layers, testability, test-suite-quality, reproducible-builds, delivery-performance, observability, pstack, pe-engineering]
status: draft
sources:
  - id: testing-theory
...
The engineering-system capability row is the one whose members are owned outside
this framework, and each has a named home in this bundle.
...
| Delivery automation and the pipelines that run a task | [Automation](../../delivery/automation/), under How to ship it |
...
| Domain modeling, boundary discipline, type discipline, idempotency, and deletion | Design principles | No destination yet; the Architecture and technical design area of [How to build it](../) is unwritten, and these principles bear on the correctness, reliability, evolvability, and intelligibility pillars without belonging to any of them |
| Root-cause analysis and TDD | Inspection or change methods | [Review aids](review-aids/) for the inspection half, selected when their preconditions hold; the change method itself is outside this bundle[^pstack-tdd] |
| Direct artifact checks and regression evidence | Assurance mechanisms and evidence practice | The Assurance and Evidence [cross-cutting records](cross-cutting-concern-records.md) |
| Multi-model adversarial review and lead synthesis | Review protocol | [Model-assisted review](review-aids/model-assisted-review.md), as an assessment method rather than a quality criterion |
| File-size, indirection, and diff-size thresholds | Contextual heuristics or proxies | [Repository evidence](review-aids/repository-evidence.md), where a proxy requires calibration and is never sufficient proof |

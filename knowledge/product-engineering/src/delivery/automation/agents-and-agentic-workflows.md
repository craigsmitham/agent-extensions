...
tags: [agents, agentic-workflows, llm-workflows, automation, control-flow, durability, orchestration, pe-delivery]
status: stable
sources:
  - id: anthropic-agents
...
The fourth column names concerns this subtree does not own. Agent engineering is
the design of goal-directed model-driven systems, and the separate
`agent-engineering` bundle owns it; each row states the concerns inline so the
table reads without that bundle installed.

...
tags: [workflow, model, taxonomy, pipeline, job, step, runner, vendor-mapping, pe-delivery]
status: draft
...
The table below is the only place this bundle names vendors at length. It is a
comparative mapping, not a binding: it teaches nobody how to configure a
platform, and it earns its place by showing that the vendor nouns do not agree,
which is the observation the portable model above answers. It is held to a
currency contract rather than to the bundle's usual technology-agnostic rule,
and the terms are in [the overview](../../overview.md). The `stale_after` date
in this file's frontmatter is that contract in force. On reaching it, recheck
the cited platform documentation and either refresh the table and move the date,
or remove the table and keep the model, which stands without it. A stale mapping
is worse than no mapping, because it invites a reader to trust a hierarchy that
has since changed.

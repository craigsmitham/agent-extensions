...
tags: [workflow, practice, continuous-delivery, release, deployment, pe-delivery]
status: draft
sources:
...
1. A [pipeline](pipeline-explainer.md) accumulates evidence as a
   candidate progresses.
2. [Build once and promote](build-once-promote-explainer.md) keeps
   the identity of that candidate stable across validation and targets.
3. [Quality gates](quality-gate-explainer.md) make progression
